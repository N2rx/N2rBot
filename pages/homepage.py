...
from sites.neweggcombo import NewEggcombo
from pages.createdialog import CreateDialog
from pages.pollbrowser import PollBrowserDialog
from utils import get_profile, get_account, get_proxy, BirdLogger, return_data, write_data
...
class HomePage(QtWidgets.QWidget):
    ...
    def setupUi(self, homepage):
        ...
        self.account_table_header = QtWidgets.QLabel(self.tasks_card)
        self.account_table_header.setGeometry(QtCore.QRect(785, 7, 61, 31))
        self.account_table_header.setFont(font)
        self.account_table_header.setStyleSheet("color: rgb(234, 239, 239);border: none;")
        self.account_table_header.setText("Account")
    ...
    def load_tasks(self):
        ...
        try:
            for task in tasks_data:
                tab = TaskTab(task["site"],task["product"],task["profile"],task["account"],task["proxies"],task["monitor_delay"],task["error_delay"],task["max_price"],task["end_less"],task["cookie_name"],self.stop_all_tasks,self.scrollAreaWidgetContents)
...
class TaskTab(QtWidgets.QWidget):
    def __init__(self,site,product,profile,account,proxies,monitor_delay,error_delay,max_price,end_less,cookie_name,stop_all,parent=None):
        super(TaskTab, self).__init__(parent)
        self.task_id = str(int(tasks_total_count.text())+1)
        tasks_total_count.setText(self.task_id)
        self.site,self.product,self.profile,self.account,self.proxies,self.monitor_delay,self.error_delay,self.max_price,self.end_less,self.cookie_name,self.stop_all = site,product,profile,account,proxies,monitor_delay,error_delay,max_price,end_less,cookie_name,stop_all
        self.setupUi(self)
        tasks.append(self)
        tasks_data = return_data("./data/tasks.json")
        task_data = {"task_id": self.task_id,"site":self.site,"product": self.product,"profile": self.profile,"account": self.account,"proxies": self.proxies,"monitor_delay": self.monitor_delay,"error_delay": self.error_delay,"max_price": self.max_price,"end_less": self.end_less,"cookie_name": self.cookie_name}
    ...
    def setupUi(self,TaskTab):
        ...
        self.account_label = QtWidgets.QLabel(self.TaskTab)
        self.account_label.setGeometry(QtCore.QRect(765, 10, 51, 31))
        self.account_label.setFont(font)
        self.account_label.setStyleSheet("color: rgb(234, 239, 239);")
        ...
        self.cookie_name_label = QtWidgets.QLabel(self.TaskTab)
        self.cookie_name_label.hide()
    ...
    def load_labels(self):
        ...
        self.cookie_name_label.setText(self.cookie_name)
    ...
    def update_status(self,msg):
        ...
        if msg["status"] == "idle":
            ...
            logger.normal(self.task_id,self.account,msg["msg"])
            ...
            logger.alt(self.task_id,self.account,msg["msg"])
            ...
            logger.success(self.task_id,self.account,msg["msg"])
            self.running = False
            self.start_btn.raise_()
            if settings.buy_one:
                self.task.stop()
            ...
            self.status_label.setStyleSheet("color: rgb(163, 149, 255);")
            logger.carted(self.task_id,self.account,msg["msg"])
    ...
    def edit(self,event):
        ...
        self.edit_dialog.account_box.clear()
        ...
        for account in [account_combobox.itemText(i) for i in range(account_combobox.count())]:
            self.edit_dialog.account_box.addItem(account)
    ...
    def update_task(self):
        ...
        self.account=self.edit_dialog.account_box.currentText()
        ...
        self.end_less = self.edit_dialog.endless_checkbox.isChecked()
        self.cookie_name = self.edit_dialog.cookiename_edit.text()
        self.load_labels()
        self.delete_json()
        tasks_data = return_data("./data/tasks.json")
        task_data = {"task_id": self.task_id, "site": self.site, "product": self.product, "profile": self.profile,
                     "account": self.account, "proxies": self.proxies, "monitor_delay": self.monitor_delay, "error_delay": self.error_delay,
                     "max_price": self.max_price, "end_less": self.end_less, "cookie_name": self.cookie_name}
        tasks_data.append(task_data)
        write_data("./data/tasks.json",tasks_data)
...
class TaskThread(QtCore.QThread):
    ...
    def set_data(self,task_id,site,product,profile,account,proxies,monitor_delay,error_delay,max_price,end_less,cookie_name):
        self.task_id,self.site,self.product,self.profile,self.account,self.proxies,self.monitor_delay,self.error_delay,self.max_price,self.end_less,self.cookie_name = task_id,site,product,profile,account,proxies,monitor_delay,error_delay,max_price,end_less,cookie_name

    def run(self):
        profile,account,proxy = get_profile(self.profile),get_account(self.account),get_proxy(self.proxies)
        if profile is None:
        ...
        if proxy is None:
            ...
            return
        if account is None:
            self.status_signal.emit({"msg": "Invalid account", "status": "error"})
            
        elif self.site == "Newegg Saudi":
            NewEgg(self.task_id, self.status_signal, self.image_signal, self.product, profile, account, proxy, self.monitor_delay, self.error_delay, self.end_less, self.cookie_name)
        elif self.site == "Newegg combo Saudi":
            NewEggcombo(self.task_id, self.status_signal, self.image_signal, self.product, profile, account, proxy, self.monitor_delay, self.error_delay, self.end_less, self.cookie_name)
