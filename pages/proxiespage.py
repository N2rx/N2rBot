...
class ProxiesPage(QtWidgets.QWidget):
    ...
    def setupUi(self, proxiespage):
        ...
        self.proxies_card.setStyleSheet("background-color: {};border-radius: 20px;border: 1px solid #2e2d2d;".format(globalStyles["backgroundLight"]))
        ...
        self.listname_edit.setStyleSheet("outline: 0;border: 1px solid {};border-width: 0 0 2px;color: rgb(234, 239, 239);".format(globalStyles["primary"]))
        ...
        self.loadlist_box.setStyleSheet("outline: 0;border: 1px solid {};border-width: 0 0 2px;color: rgb(234, 239, 239);".format(globalStyles["primary"]))
        ...
        self.saveproxies_btn.setStyleSheet("color: #FFFFFF;background-color: {};border-radius: 10px;border: 1px solid #2e2d2d;".format(globalStyles["primary"]))
        ...
        self.deleteproxies_btn.setStyleSheet("color: #FFFFFF;background-color: {};border-radius: 10px;border: 1px solid #2e2d2d;".format(globalStyles["primary"]))
        ...
        if proxies != "" and list_name != "":
            for item in proxies.splitlines():
                if ":" not in item or item == "":
                    QtWidgets.QMessageBox.critical(self, "N2r Bot", "Incorrect Proxies")
                    return
            proxies_data = {
                "list_name": list_name,
                "proxies": self.proxies_edit.toPlainText()
            ...
            if self.loadlist_box.findText(list_name) == -1:
                self.loadlist_box.addItem(list_name)
                self.parent().parent().createdialog.proxies_box.addItem(list_name)
            QtWidgets.QMessageBox.information(self, "N2r Bot", "Saved Proxies")
        else:
            QtWidgets.QMessageBox.critical(self, "N2r Bot", "Missing Fields")
    ...
    def delete_proxies(self):
        ...
        QtWidgets.QMessageBox.information(self, "N2r Bot", "Deleted Proxy List")